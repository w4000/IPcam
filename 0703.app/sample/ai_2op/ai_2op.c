...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < NN_RUN_OP_NUM; i++) {
		...
		else if (stream[i].net_op_opt == AI_OP_PREPROC_Y2Y_UV2UV)
			printf("OP%u: Run PREPROC (Y2Y_UV2UV)!\r\n",i);
		else if (stream[i].net_op_opt == AI_OP_FC_LL_MODE)
			printf("OP%u: Run FC (LL MODE)!\r\n",i);
		else {
	...
	for (i = 0; i < NN_RUN_OP_NUM; i++) {
    	if(stream[i].net_op_opt != AI_OP_FC && stream[i].net_op_opt != AI_OP_FC_LL_MODE) { //for preproc
        	// set open config
