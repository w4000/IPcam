...
// Global variable
//============================================================================
PKDRV_SIE_DRV_INFO pkdrv_info_data;
...
//=============================================================================
static int nvt_kdrv_sie_seq_printf(const char *fmtstr, ...)
{
	va_list marker;
...
static int nvt_kdrv_sie_proc_info_show(struct seq_file *sfile, void *v)
{
	pkdrv_info_data->info_seq_file = sfile;
	kdrv_sie_dump_info(nvt_kdrv_sie_seq_printf);
	pkdrv_info_data->info_seq_file = NULL;
	return 0;
}

static int nvt_kdrv_sie_proc_info_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvt_kdrv_sie_proc_info_show, NULL);
}

static struct file_operations proc_info_fops = {
	.owner  = THIS_MODULE,
	.open   = nvt_kdrv_sie_proc_info_open,
...
{
    return single_open(file, nvt_kdrv_sie_proc_cmd_show, &pkdrv_info_data->module_info);
}
