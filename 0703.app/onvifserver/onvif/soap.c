...
#include "cam_setup.h"
#define printf_function()//printf("w4000 %s, %d \n",__FUNCTION__,__LINE__)//w4000
...
void print_httpmsg(HTTPMSG *rx_msg)//w4000_info
{
    if (!rx_msg)
    {
        printf("rx_msg is NULL\n");
        return;
    }
    ...
    // 첫 라인 출력 (메서드 + 경로/프로토콜)
    if (rx_msg->first_line.header[0] != '\0' && rx_msg->first_line.value_string)
    {
        printf("first_line:   %s %s\n", rx_msg->first_line.header, rx_msg->first_line.value_string);
    }
    else
    {
        printf("first_line:   (NULL)\n");
    }

    printf("hdr_len:      %d\n", rx_msg->hdr_len);
    printf("ctt_len:      %d\n", rx_msg->ctt_len);
    printf("ctt_type:     %d\n", rx_msg->ctt_type);
    printf("boundary:     %s\n", rx_msg->boundary[0] ? rx_msg->boundary : "(none)");

    // Remote IP 출력
    printf("remote_ip:    %u.%u.%u.%u\n",
           (rx_msg->remote_ip >> 24) & 0xFF,
           (rx_msg->remote_ip >> 16) & 0xFF,
           (rx_msg->remote_ip >> 8) & 0xFF,
           rx_msg->remote_ip & 0xFF);
    printf("remote_port:  %u\n", rx_msg->remote_port);

    // 전체 메시지 출력
    if (rx_msg->msg_buf && rx_msg->buf_offset > 0)
    {
        printf("msg_buf (%d bytes):\n", rx_msg->buf_offset);
        printf("%.*s\n", rx_msg->buf_offset, rx_msg->msg_buf);
    }
    else
    {
        printf("msg_buf: (empty)\n");
    }
...
int soap_process_device_request(HTTPCLN * p_user, HTTPMSG * rx_msg, XMLN * p_body, XMLN * p_header)
{
    ...
    const char * p_name;
				...
				{		  
					return soap_err_rly(p_user, rx_msg,ERR_SENDER,ERR_NOTAUTHORIZED,NULL,"Sender not Authorized",NULL,401,"Unauthorized");	
				}	
#endif
#if 0//is_auth_w4000
	{
		const char *path = "/mnt/ipm/login_status.txt";
	
		if (access(path, F_OK) == 0)
		{
			printf("w4000 file exist: %s\n", path);
		}
		else
		{
			printf("w4000 file not exist: %s\n", path);
		}
...
/*
void base64_encode(const unsigned char* buffer, size_t length, char** base64_text)
{
	BIO *bio, *b64;
	BUF_MEM *bufferPtr;
	...
	*base64_text = (char *)malloc((bufferPtr->length + 1) * sizeof(char));
...
void generate_password_digest(const char* nonce_b64, const char* created, const char* password, char** out_digest_b64)
{
	// Base64 decode nonce
	BIO *b64 = BIO_new(BIO_f_base64());
	BIO *bmem = BIO_new_mem_buf((void*)nonce_b64, -1);
	bmem = BIO_push(b64, bmem);
	BIO_set_flags(bmem, BIO_FLAGS_BASE64_NO_NL);
	unsigned char nonce_decoded[64];
	int nonce_len = BIO_read(bmem, nonce_decoded, sizeof(nonce_decoded));
	BIO_free_all(bmem);
	...
	memcpy(buffer + total_len, nonce_decoded, nonce_len);
	total_len += nonce_len;
	strcpy((char*)buffer + total_len, created);
	total_len += strlen(created);
	strcpy((char*)buffer + total_len, password);
	total_len += strlen(password);

	// SHA1 hash
	unsigned char sha1_digest[SHA_DIGEST_LENGTH];
	SHA1(buffer, total_len, sha1_digest);

	// Base64 encode the result
	base64_encode(sha1_digest, SHA_DIGEST_LENGTH, out_digest_b64);
}

#if 0
// return 1=0k
int parse_username_token1(const char *xml) {//w4000
    const char *start_tag = "<UsernameToken>";
    const char *end_tag = "</UsernameToken>";
    
    char *start = strstr(xml, start_tag);
    char *end = strstr(xml, end_tag);

    if (start && end && start < end) {
        start += strlen(start_tag); // start를 UsernameToken의 끝으로 이동
        size_t length = end - start; // UsernameToken 내용의 길이
        
        char *username_token = (char *)malloc(length + 1);
        if (username_token) {
            strncpy(username_token, start, length);
            username_token[length] = '\0'; // 문자열 종료

            printf("--------------------------------------\n Parsed UsernameToken: %s\n", username_token);
            free(username_token); // 메모리 해제
			{
				char* digest_b64 = NULL;
				//이 부 분 수정 
				generate_password_digest("bOTFOvCb+keX5QJ0FXVDsR4AAAAAAA==", "2025-04-24T09:41:02.327Z", "1111", &digest_b64);
        ...
        printf("UsernameToken not found.\n");
		return 0;
    }
}
#else
...
#include <stdlib.h>
#include <string.h>
#include <stdint.h> // For uint32 and uint8 types
#include <openssl/sha.h>
...
// Base64 디코딩 함수
unsigned char* w4000_base64_decode(const unsigned char *src, size_t len, size_t *out_len) {
    if (!src || !out_len || len == 0) {
        return NULL;
    }

    size_t max_output_size = (len / 4) * 3 + 3;
    ...
    *out_len = 0;
    ...
    while (i < len) {
        ...
        a = base64_decode_table[src[i]];
        b = base64_decode_table[src[i + 1]];
        c = base64_decode_table[src[i + 2]];
        d = base64_decode_table[src[i + 3]];
...
// PasswordDigest 검증 함수
int verify_password_digest(const char* nonce_base64, const char* created, 
                         const char* password, const char* digest_base64) {
    size_t nonce_len, digest_len;
    
    // Nonce 디코딩
    unsigned char* nonce = w4000_base64_decode((const unsigned char*)nonce_base64, 
                                          strlen(nonce_base64), &nonce_len);
    if (!nonce) return 0;

    // Digest 디코딩
    unsigned char* received_digest = w4000_base64_decode((const unsigned char*)digest_base64, 
                                                    strlen(digest_base64), &digest_len);
    if (!received_digest) {
        free(nonce);
        return 0;
    }

    // SHA1 해시 계산을 위한 버퍼 준비
    size_t total_len = nonce_len + strlen(created) + strlen(password);
    unsigned char* buffer = malloc(total_len);
    if (!buffer) {
        free(nonce);
        free(received_digest);
        return 0;
    }

    // 버퍼에 Nonce + Created + Password 연결
    memcpy(buffer, nonce, nonce_len);
    memcpy(buffer + nonce_len, created, strlen(created));
    memcpy(buffer + nonce_len + strlen(created), password, strlen(password));

    // SHA1 해시 계산
    unsigned char calculated_digest[SHA_DIGEST_LENGTH];
    SHA1(buffer, total_len, calculated_digest);

    // 계산된 해시와 받은 다이제스트 비교
    int result = (digest_len == SHA_DIGEST_LENGTH && 
                 memcmp(calculated_digest, received_digest, SHA_DIGEST_LENGTH) == 0);
...
int main1() 
{
    const char* nonce_base64 = "DPoWMeBnGky2hf2WreGw6xoAAAAAAA==";
    const char* created = "2025-04-25T15:45:32Z";
    const char* password = "admin123";
    const char* digest_base64 = "3//vpwz7kWh0tqwpPztbdp/9VlE=";

    if (verify_password_digest(nonce_base64, created, password, digest_base64)) {
        printf("Password digest verification successful!\n");
    } else {
        printf("Password digest verification failed!\n");
    }

    return 0;
}
// Modified parse_username_token1 function
int parse_username_token1(const char *xml, const char* input_id, const char* input_password) {
    const char *start_tag = "<UsernameToken>";
    const char *end_tag = "</UsernameToken>";
    ...
    if (start && end && start < end) {
        start += strlen(start_tag); // Move start to the end of <UsernameToken>
        size_t length = end - start; // Length of <UsernameToken> contents

        char *username_token = (char *)malloc(length + 1);
        if (username_token) {
            strncpy(username_token, start, length);
            username_token[length] = '\0'; // Null-terminate the string

            printf("--------------------------------------\n Parsed UsernameToken: %s\n", username_token);

            // Extract <Username>
            const char *username_start_tag = "<Username>";
            const char *username_end_tag = "</Username>";
            char *username_start = strstr(username_token, username_start_tag);
            char *username_end = strstr(username_token, username_end_tag);
            char extracted_id[128] = {0};

            if (username_start && username_end) {
                username_start += strlen(username_start_tag); // Move to the end of <Username>
                size_t username_len = username_end - username_start;
                strncpy(extracted_id, username_start, username_len);
                extracted_id[username_len] = '\0';
            }

            // Extract <Password>
            const char *password_start_tag = "<Password";
            const char *password_end_tag = "</Password>";
            char *password_start = strstr(username_token, password_start_tag);
            char *password_end = strstr(username_token, password_end_tag);
            char extracted_password_base64[256] = {0};

            if (password_start && password_end) {
                password_start = strstr(password_start, ">") + 1; // Move to the content inside <Password>
                size_t password_len = password_end - password_start;
                strncpy(extracted_password_base64, password_start, password_len);
                extracted_password_base64[password_len] = '\0';
            }

            // Decode Base64 password using onvif_base64_decode
		size_t decoded_length = 0;
		unsigned char *decoded_password = w4000_base64_decode((unsigned char*)extracted_password_base64, strlen(extracted_password_base64), &decoded_length);
			
		if (decoded_password) 
		{ // 성공 시	
				decoded_password[decoded_length] = '\0'; // Null-terminate the decoded password
                // Compare extracted ID and decoded password with input values
                if (strcmp(extracted_id, input_id) == 0 && strcmp((char*)decoded_password, input_password) == 0) 
				{
                    printf("ID and Password match. Authentication successful.\n");
                    free(username_token); // Free allocated memory
                    return 1; // Success
                } 
				else 
                {
                    printf("ID or Password mismatch. Authentication failed.\n");
				...
				printf("Base64 decoding failed. Decoded Length: %d\n", 0);
                free(username_token);
                return 0;
            }
        } else {
...
int parse_username_token2(const char *xml, const char* input_id, const char* input_password) {
    const char *start_tag = "<UsernameToken>";
    const char *end_tag = "</UsernameToken>";
    ...
    if (start && end && start < end) {
        ...
        if (username_token) {
            ...
            if (username_start && username_end) {
                username_start += strlen(username_start_tag);
                size_t username_len = username_end - username_start;
                strncpy(extracted_id, username_start, username_len);
                extracted_id[username_len] = '\0';
            }

            // Extract PasswordDigest
            const char *password_start_tag = "<Password";
            const char *password_end_tag = "</Password>";
            char *password_start = strstr(username_token, password_start_tag);
            char *password_end = strstr(username_token, password_end_tag);
            char extracted_digest[256] = {0};

            if (password_start && password_end) {
                password_start = strstr(password_start, ">") + 1;
                size_t password_len = password_end - password_start;
                strncpy(extracted_digest, password_start, password_len);
                extracted_digest[password_len] = '\0';
            }

            // Extract Nonce
            const char *nonce_start_tag = "<Nonce";
            const char *nonce_end_tag = "</Nonce>";
            char *nonce_start = strstr(username_token, nonce_start_tag);
            char *nonce_end = strstr(username_token, nonce_end_tag);
            char extracted_nonce[256] = {0};

            if (nonce_start && nonce_end) {
                nonce_start = strstr(nonce_start, ">") + 1;
                size_t nonce_len = nonce_end - nonce_start;
                strncpy(extracted_nonce, nonce_start, nonce_len);
                extracted_nonce[nonce_len] = '\0';
            }

            // Extract Created timestamp
            const char *created_start_tag = "<Created";
            const char *created_end_tag = "</Created>";
            char *created_start = strstr(username_token, created_start_tag);
            char *created_end = strstr(username_token, created_end_tag);
            char extracted_created[64] = {0};

            if (created_start && created_end) {
                created_start = strstr(created_start, ">") + 1;
                size_t created_len = created_end - created_start;
                strncpy(extracted_created, created_start, created_len);
                extracted_created[created_len] = '\0';
            }

            // First verify the username
            if (strcmp(extracted_id, input_id) != 0) {
                printf("Username mismatch\n");
                free(username_token);
                return 0;
            }

            // Verify the password digest
            int result = verify_password_digest(
                extracted_nonce,    // Base64 encoded nonce
                extracted_created,  // Created timestamp
                input_password,     // Plain password
                extracted_digest    // Base64 encoded digest to verify
            );
...
/*
int main() {
    const char *xml = "<UsernameToken><Username>1111</Username><Password Type=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-username-token-profile-1.0#PasswordDigest\">1mHvki0nMGqCnmbFvvk+nbz3c9M=</Password><Nonce EncodingType=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-soap-message-security-1.0#Base64Binary\">bOTFOvCb+keX5QJ0FXVDsSgAAAAAAA==</Nonce><Created xmlns=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd\">2025-04-24T10:02:15.760Z</Created></UsernameToken>";
    const char *id = "1111";
    const char *password = "1111"; // Raw password

    int result = parse_username_token(xml, id, password);
    if (result) {
        printf("Authentication successful.\n");
    } else {
        printf("Authentication failed.\n");
...
void process_request(HTTPCLN * p_user, HTTPMSG * rx_msg)
{
    ...
    {
        //if (!check_http_auth(p_cln, p_msg))
        {
            //send_401_unauthorized(p_cln);
			printf("w4000 %s,need_auth =%d \n",__FUNCTION__,g_onvif_cfg.need_auth);
            return;
        }
    }
	...
	//if(parse_username_token(p_xml)==0)

	
		{//is_auth_w4000 w4000_xml
		char user[256];
		char pswd[256];
					int i, ok = 0;
					for (i=0; i<10; i++)   
					{
						...
						{
							{
								sprintf(user, "%s", SETUP_USR_NAME(&g_setup, i));
								sprintf(pswd, "%s", SETUP_USR_PWD(&g_setup, i));
		
									//int result = parse_username_token2(p_xml, "admin", "1234");
									int result = parse_username_token2(p_xml, user, pswd);
								    if (result) {
								        printf("Authentication successful.\n");
										set_auth_w4000(1);
										break;
    ...
    else if (strstr(p_post, "device"))//is_auth_w4000
    {
