...
BOOL ctl_sen_cmd_info(unsigned char argc, char **pargv)
{
	UINT32 i;
	...
	for (i = 0; i < CTL_SEN_ID_MAX; i++) {
		...
		sen_dbg_get_obj()->dump_info(vk_printk, i);
		sen_dbg_get_obj()->dump_ext_info(vk_printk, i);
		sen_dbg_get_obj()->dump_drv_info(vk_printk, i);
		sen_dbg_get_obj()->dump_ctl_info(vk_printk, i);
	}
	sen_dbg_get_obj()->dump_map_tbl_info(vk_printk);
	sen_dbg_get_obj()->dump_proc_time(vk_printk);

	return 0;
}

BOOL ctl_sen_cmd_signal(unsigned char argc, char **pargv)
{
	UINT32 i, j;
	...
	for (i = 0; i < CTL_SEN_ID_MAX; i++) {
		...
		if (g_ctl_sen_ctrl_obj[i]->cur_chgmode.output_dest == CTL_SEN_OUTPUT_DEST_AUTO) {
			if (i < CTL_SEN_MAX_OUTPUT_SIE_IDX) {
				sen_dbg_get_obj()->wait_inte(vk_printk, i, inte_vd[i]);
				sen_dbg_get_obj()->wait_inte(vk_printk, i, inte_fmd[i]);
			} else {
				sen_dbg_get_obj()->wait_inte(vk_printk, i, inte_vd[0]);
				sen_dbg_get_obj()->wait_inte(vk_printk, i, inte_fmd[0]);
			}
		} else {
			for (j = 0; j < CTL_SEN_MAX_OUTPUT_SIE_IDX; j++) {
				if (g_ctl_sen_ctrl_obj[i]->cur_chgmode.output_dest & (1 << j)) {
					sen_dbg_get_obj()->wait_inte(vk_printk, i, inte_vd[j]);
					sen_dbg_get_obj()->wait_inte(vk_printk, i, inte_fmd[j]);
				}
			}
		}
	}
...
SXCMD_ITEM("info",        	ctl_sen_cmd_info,               "dump information")
SXCMD_ITEM("signal",        ctl_sen_cmd_signal,             "dump signal")
