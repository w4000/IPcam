...
char SESSION_COOKIE_NAME[50] = "ISESSIONSFCGIID";

int sess_initialized = 1;; // true if the session has been initialized, otherwise false
...
* \endcode  
**/
int fcgi_add_cookie(const char *name, 
	const char *value, 
	const char *max_age,
	const char *path, 
	const char *domain, 
	const int secure, FCGX_Stream *out)
{
	if (headers_initialized)
		return 0;

	FCGX_FPrintF(out, "Set-cookie: %s=%s;", name, value);

	if (max_age)
		FCGX_FPrintF(out, " Max-age=%s;", max_age);
	if (path)
		FCGX_FPrintF(out, " Path=%s;", path);
	if (domain)
		FCGX_FPrintF(out, " Domain=%s;", domain);
	if (secure)
		FCGX_FPrintF(out, " Secure");
	FCGX_FPrintF(out, "\r\n");
...
formvars *fcgi_get_cookies(FCGX_ParamArray envp)
{
	...
	if ((c = FCGX_GetParam("HTTP_COOKIE", envp)) == NULL)
		return NULL;
...
int fcgi_session_destroy(FCGX_Stream *out)
{
	...
	if (!unlink(sess_fname)) {
		...
		if (headers_initialized) 
			...
			fcgi_add_cookie(SESSION_COOKIE_NAME, "", 0, 0, 0, 0, out);
...
*/
char *fcgi_session_var(const char *var_name)
{
...
int fcgi_session_register_var(const char *name, const char *value)
{
	...
	if (!fcgi_session_var_exists(name)) {
...
int fcgi_session_start(FCGX_ParamArray envp, FCGX_Stream *out)
{
	...
	// Get the session ID
	sid = fcgi_cookie_value(SESSION_COOKIE_NAME);

	// If there isn't a session ID, we need to create one 
	if (sid == NULL) {
		if (sess_create_file()) {
			fcgi_add_cookie(SESSION_COOKIE_NAME, sess_id, 0, 0, 0, 0, out);
