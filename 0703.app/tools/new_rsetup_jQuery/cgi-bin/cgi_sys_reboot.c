...
static void replace_resource_string(char* page, int lang)
{
	...
	char* szRES_REBOOT_CONFIRM[] = RES_ID_ARRAY(RES_REBOOT_CONFIRM);
	char* szRES_MSG_REBOOT[] = RES_ID_ARRAY(RES_MSG_REBOOT);
	...
	replace_form(page, "##RES_REBOOT_CONFIRM##", szRES_REBOOT_CONFIRM[lang]);
	replace_form(page, "##RES_MSG_REBOOT##", szRES_MSG_REBOOT[lang]);

	char* szRES_WARNING[] = RES_ID_ARRAY(RES_WARNING);
	char* szRES_MSG_REBOOT_1[] = RES_ID_ARRAY(RES_MSG_REBOOT_1);
	replace_form(page, "##RES_WARNING##", szRES_WARNING[lang]);
	replace_form(page, "##RES_MSG_REBOOT_1##", szRES_MSG_REBOOT_1[lang]);
...
retry:
	load_form(JQUERY_HTML_SETUP_AJAX_SYSTEM_REBOOT, page, lang);
