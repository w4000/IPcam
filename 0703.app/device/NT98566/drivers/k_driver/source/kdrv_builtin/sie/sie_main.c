...
static int nvt_sie_probe(struct platform_device * pdev)
{
	...
	UINT32 efuse_addr = 0xf0660074, efuse_addr_map_size = 0x20;// efuse address
	UINT32 cg_base_addr = 0xf0020000, cg_base_addr_map_size = 0x90;// CG base-address
	...
	// for external io address0 (efuse address)
	pdrv_info->module_info.ext_io_addr0 = ioremap_nocache(efuse_addr, efuse_addr_map_size);
	if (pdrv_info->module_info.ext_io_addr0 == NULL) {
		nvt_dbg(ERR, "ext_io_addr0 ioremap() failed in module sie\n");
		iounmap(pdrv_info->module_info.ext_io_addr0);
		rt = -1;
		goto FAIL_FREE_RES;
	}

	// for external io address1 (cg address)
	pdrv_info->module_info.ext_io_addr1 = ioremap_nocache(cg_base_addr, cg_base_addr_map_size);
	if (pdrv_info->module_info.ext_io_addr1 == NULL) {
		nvt_dbg(ERR, "ext_io_addr0 ioremap() failed in module sie\n");
		iounmap(pdrv_info->module_info.ext_io_addr1);
		rt = -1;
		goto FAIL_FREE_RES;
	}
