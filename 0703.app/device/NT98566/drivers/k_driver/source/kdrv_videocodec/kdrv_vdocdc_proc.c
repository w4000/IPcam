...
{
	int i=0;
	H26XEncMAQDiffInfoCfg stMAQDiffInfoCfg = {0};

	seq_printf(sfile, "usage: echo [channel] [enable] [str] [diff_idx_1] [diff_idx_2] > /proc/kdrv_vdocdc/diff_maq\n");
	for (i = 0; i < vdoenc_max_channel; i++) {
		if (g_enc_info[i].b_enable == TRUE) {
			h26XEnc_getMAQDiffInfoCfg(&g_enc_info[i].enc_var, &stMAQDiffInfoCfg);
			seq_printf(sfile, "==== current value: channel %d, enable %d, str %d, idx_1 %d, idx_2 %d====\n", i, stMAQDiffInfoCfg.enable, stMAQDiffInfoCfg.str, stMAQDiffInfoCfg.start_idx, stMAQDiffInfoCfg.end_idx);
		}
	}
	return 0;
}
...
{
    int channel, enable, str, idx_1, idx_2;
    char proc_buffer[PROC_MAX_BUFFER];
	H26XEncMAQDiffInfoCfg stMAQDiffInfoCfg = {0};
    ...
    sscanf(proc_buffer, "%d %d %d %d %d ", &channel, &enable, &str, &idx_1, &idx_2);

	if (g_enc_info[channel].b_enable == TRUE) {
		stMAQDiffInfoCfg.enable = enable;
		stMAQDiffInfoCfg.str = str;
		stMAQDiffInfoCfg.start_idx = idx_1;
		stMAQDiffInfoCfg.end_idx = idx_2;

		h26XEnc_setMAQDiffInfoCfg(&g_enc_info[channel].enc_var, &stMAQDiffInfoCfg);
	} else {
		DBG_ERR("channel is not available\n");
	}
