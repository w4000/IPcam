...
INT32 kdrv_ssenif_open(UINT32 chip, UINT32 engine)
{
...
#if SSENIF_CSI_EN
		dal_engid = KDRV_SSENIF_ENGINE_CSI2 + engine - KDRV_SSENIF_ENGINE_LVDS0;	// LVDS engine id = 0x9002 & 0x9003
#else
		dal_engid = KDRV_SSENIF_ENGINE_CSI0 + engine - KDRV_SSENIF_ENGINE_LVDS0;
#endif
	} else {
		DBG_ERR("Err engine\r\n");
		return -1;
	...
	object = dal_ssenif_get_object((DAL_SSENIF_ID)(dal_engid & 0xFF));
