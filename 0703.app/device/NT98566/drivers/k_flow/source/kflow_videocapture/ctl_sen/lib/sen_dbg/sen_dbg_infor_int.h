...
typedef struct {
	void(*dump_info)(int (*dump)(const char *fmt, ...), CTL_SEN_ID id);                         ///< dump get info (PCTL_SEN_OBJ()->get_cfg)
	void(*dump_ext_info)(int (*dump)(const char *fmt, ...), CTL_SEN_ID id);                     ///< dump ext info (PCTL_SEN_OBJ()->init_cfg & PCTL_SEN_OBJ()->get_cfg(CTL_SEN_CFGID_INIT_XX, value))
	void(*dump_drv_info)(int (*dump)(const char *fmt, ...), CTL_SEN_ID id);                     ///< dump sensor driver get info (CTL_SEN_DRV_TAB()->get_cfg(id, CTL_SENDRV_CFGID, value))
	void(*wait_inte)(int (*dump)(const char *fmt, ...), CTL_SEN_ID id, CTL_SEN_INTE inte);      ///< wait sen_ctrl_if_get_obj inter. only support serial sensor (LVDS/CSI/SLVS-EC)
	void(*dump_map_tbl_info)(int (*dump)(const char *fmt, ...));                                ///< dump (ctl_sen_reg_sendrv/ctl_sen_unreg_sendrv & PCTL_SEN_OBJ()->init_cfg) mapping table
	void(*dump_proc_time)(int (*dump)(const char *fmt, ...));                                   ///< dump ctl sen process time
	void(*dbg_msg)(CTL_SEN_ID id, BOOL en, CTL_SEN_MSG_TYPE type);                              ///< enable debug msg
	void(*dump_ctl_info)(int (*dump)(const char *fmt, ...), CTL_SEN_ID id);                     ///< dump ctl_sen inner info
