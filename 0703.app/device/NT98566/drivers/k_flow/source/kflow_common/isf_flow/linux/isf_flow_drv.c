...
int isf_flow_drv_ioctl(unsigned char minor, PMODULE_INFO pmodule_info, unsigned int cmd_id, unsigned long arg)
{
	...
	case ISF_FLOW_CMD_SET_BIND: {
			...
			p_cmd->rv = isf_unit_set_bind(minor, p_cmd->src, p_cmd->dest); //NOTE: p_cmd->sid is not used
	...
	case ISF_FLOW_CMD_GET_BIND: {
			...
			p_cmd->rv = isf_unit_get_bind(minor, p_cmd->src, &(p_cmd->dest)); //NOTE: p_cmd->sid is not used
	...
	case ISF_FLOW_CMD_SET_STATE: {
			...
			p_cmd->rv = isf_unit_set_state(minor, p_cmd->src, p_cmd->state); //NOTE: p_cmd->sid is not used
	...
	case ISF_FLOW_CMD_GET_STATE: {
			...
			p_cmd->rv = isf_unit_get_state(minor, p_cmd->src, &(p_cmd->state)); //NOTE: p_cmd->sid is not used
	...
	case ISF_FLOW_CMD_SET_PARAM: {
			...
			if(p_cmd->size > 0) {
				...
				p_cmd->rv = isf_unit_set_struct(minor, p_cmd->dest, p_cmd->param, (UINT32*)p_struct, p_cmd->size);
				kfree(p_struct);
				//p_struct = NULL;
			} else {
				p_cmd->rv = isf_unit_set_param(minor, p_cmd->dest, p_cmd->param, p_cmd->value);
			}
	...
	case ISF_FLOW_CMD_GET_PARAM: {
...

			{
				if(p_cmd->size > 0) {
					...
					p_cmd->rv = isf_unit_get_struct(minor, p_cmd->dest, p_cmd->param, p_struct, p_cmd->size);
					...
					p_cmd->rv = isf_unit_get_param(minor, p_cmd->dest, p_cmd->param, &p_cmd->value);
				}
			}
