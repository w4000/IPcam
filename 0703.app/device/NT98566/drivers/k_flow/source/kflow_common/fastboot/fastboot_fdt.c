	...
	// ipp fastboot register config
	{
		...
		if (dtsi_buffer) {
			...
			for (i = 0; i < 8; i++) {
				...
				snprintf(dtsi_chksum_node, 63, "/fastboot/ipp/config-%d", i);
				kdrv_ipp_builtin_get_hdal_dtsi_fdt_info(hdl_list[i], i, &fdt_get_info);

				if (update_property_u32array(fdt, dtsi_chksum_node, "chksum", &fdt_get_info.dtsi_chksum, 1) != 0) {
					DBG_ERR("failed to update_property:chksum.");
				}//else
				//	DBG_DUMP("--------------save chksum:%x success--------\n",fdt_get_info.dtsi_chksum);
