...
int isf_flow_ioctl (int fd, unsigned int cmd, void *arg)
{
	...
	case ISF_FLOW_CMD_SET_BIND: {
			...
			p_cmd->rv = isf_unit_set_bind(0, p_cmd->src, p_cmd->dest); //NOTE: p_cmd->sid is not used
	...
	case ISF_FLOW_CMD_GET_BIND: {
			...
			p_cmd->rv = isf_unit_get_bind(0, p_cmd->src, (UINT32*)&(p_cmd->dest)); //NOTE: p_cmd->sid is not used
	...
	case ISF_FLOW_CMD_SET_STATE: {
			...
			p_cmd->rv = isf_unit_set_state(0, p_cmd->src, p_cmd->state); //NOTE: p_cmd->sid is not used
	...
	case ISF_FLOW_CMD_GET_STATE: {
			...
			p_cmd->rv = isf_unit_get_state(0, p_cmd->src, (UINT32*)&(p_cmd->state)); //NOTE: p_cmd->sid is not used
	...
	case ISF_FLOW_CMD_SET_PARAM: {
			...
			if(p_cmd->size > 0) {
				p_cmd->rv = isf_unit_set_struct(0, p_cmd->dest, p_cmd->param, (UINT32*)(p_cmd->value), p_cmd->size);
			} else {
				p_cmd->rv = isf_unit_set_param(0, p_cmd->dest, p_cmd->param, p_cmd->value);
			}
	...
	case ISF_FLOW_CMD_GET_PARAM: {
			...
			{
				if(p_cmd->size > 0) {
					p_cmd->rv = isf_unit_get_struct(0, p_cmd->dest, p_cmd->param, (UINT32*)(p_cmd->value), p_cmd->size);
				} else {
					p_cmd->rv = isf_unit_get_param(0, p_cmd->dest, p_cmd->param, (UINT32*)&(p_cmd->value));
				}
			}
