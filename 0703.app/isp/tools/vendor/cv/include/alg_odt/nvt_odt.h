...
/**
     Align the newest detector result to ODT coord.
	 
	 @param[in] num: the newest detector result num.
     @param[in][out] p_rslt: the newest detector result addr.
	 @param[in] p_detcoord: the newest detector result coord.
     ...
     {
        UINT32           det_num = 1;
		odt_od_result    det_rslts[DET_MAX_OUTNUM] = {0};
		HD_URECT OdCord = {0, 0, 1920, 1080};
		...
		odt_alignodrslt(det_num, det_rslts, &OdCord);
     }
     @endcode
*/
extern HD_RESULT odt_alignodrslt(UINT32 num, odt_od_result *p_rslt, HD_URECT* p_detcoord);
