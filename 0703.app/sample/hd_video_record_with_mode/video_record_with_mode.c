...
static UINT32 g_scaleup = 0; //0(no scale up), 1(do scale up)
...
static HD_RESULT set_proc_cfg(HD_PATH_ID *p_video_proc_ctrl, HD_DIM* p_max_dim, HD_OUT_ID _out_id)
{
	...
	if (g_scaleup && g_capbind)
		video_ctrl_param.func |= HD_VIDEOPROC_FUNC_DIRECT_SCALEUP;
...
MAIN(argc, argv)
{
	...
	if (argc == 1 || argc > 8) {
		...
		printf("  <pro_scale_up> : 0(no scale up), 1(scale up)\r\n");
		return 0;
	}
	...
	if (argc > 7) {
		g_scaleup = atoi(argv[7]);
		printf("SCALE-UP %d\r\n", g_scaleup);
		if(g_scaleup > 2) {
			printf("error: not support SCALE-UP ! (%d) \r\n", g_scaleup);
			return 0;
		}
	}

    snprintf(file_path_main, 64, "/mnt/sd/dump_bs_main_%lu_%lu_%x_%lu_%x_%lu_%lu.dat", enc_type, g_capbind,(unsigned int)g_capfmt,g_prcbind,(unsigned int)g_prcfmt,g_prccfg, g_scaleup);
	...
	if(g_scaleup){
		...
		HD_VIDEOPROC_OUT video_out_size_param = {0};
			//scale
			hd_videoproc_get(stream[0].proc_path, HD_VIDEOPROC_PARAM_OUT, &video_out_size_param);
			video_out_size_param.dim.w = VDO_SIZE_W * 1.2;
			video_out_size_param.dim.h = VDO_SIZE_H * 1.2;
			ret = hd_videoproc_set(stream[0].proc_path, HD_VIDEOPROC_PARAM_OUT, &video_out_size_param);

			//crop
			video_out_param.mode  = HD_CROP_ON;
			video_out_param.win.rect.x   = VDO_SIZE_W * 0.1;
			video_out_param.win.rect.y   = VDO_SIZE_H * 0.1;
			video_out_param.win.rect.w   = VDO_SIZE_W;
			video_out_param.win.rect.h   = VDO_SIZE_H;
