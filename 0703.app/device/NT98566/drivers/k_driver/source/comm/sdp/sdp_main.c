...
//=============================================================================
#ifdef DEBUG
unsigned int sdp_debug_level = (NVT_DBG_IND | NVT_DBG_WRN | NVT_DBG_ERR);
module_param_named(sdp_debug_level, sdp_debug_level, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(sdp_debug_level, "Debug message level");
#endif

//=============================================================================
// Global variable
//=============================================================================
static struct of_device_id sdp_match_table[] = {
...
static int nvt_sdp_open(struct inode *inode, struct file *file);
static int nvt_sdp_release(struct inode *inode, struct file *file);
static long nvt_sdp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
static int nvt_sdp_probe(struct platform_device * pdev);
static int nvt_sdp_suspend( struct platform_device * pdev, pm_message_t state);
static int nvt_sdp_resume( struct platform_device * pdev);
static int nvt_sdp_remove(struct platform_device *pdev);

int __init nvt_sdp_init(void);
void __exit nvt_sdp_exit(void);
...
static int nvt_sdp_open(struct inode *inode, struct file *file)
{
	SDP_DRV_INFO* pdrv_info;

	pdrv_info = container_of(inode->i_cdev, SDP_DRV_INFO, cdev);
	file->private_data = pdrv_info;

//	if (nvt_sdp_drv_open(&pdrv_info->module_info, MINOR(inode->i_rdev))) {
...
static int nvt_sdp_release(struct inode *inode, struct file *file)
{
	...
	//nvt_sdp_drv_release(&pdrv_info->module_info, MINOR(inode->i_rdev));
    return 0;
}
...
static long nvt_sdp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	//return nvt_sdp_drv_ioctl(MINOR(inode->i_rdev), &pdrv->module_info, cmd, arg);
	return 0 ;
}

struct file_operations nvt_sdp_fops = {
    .owner   = THIS_MODULE,
    .open    = nvt_sdp_open,
    .release = nvt_sdp_release,
    .unlocked_ioctl = nvt_sdp_ioctl,
    .llseek  = no_llseek,
};
...
static int nvt_sdp_probe(struct platform_device * pdev)
{
	...
	match = of_match_device(sdp_match_table, &pdev->dev);
	if (!match){
        nvt_dbg(ERR, "Platform device not found \n");
		return -EINVAL;
	}

    pdrv_info = kzalloc(sizeof(SDP_DRV_INFO), GFP_KERNEL);
    if (!pdrv_info) {
	...
	cdev_init(&pdrv_info->cdev, &nvt_sdp_fops);
    ...
    pdrv_info->pmodule_class = class_create(THIS_MODULE, "nvt_sdp");
    ...
    /*
	ret = nvt_sdp_proc_init(pdrv_info);
	if(ret) {
	...
	ret = nvt_sdp_drv_init(&pdrv_info->module_info);
...
static int nvt_sdp_remove(struct platform_device *pdev)
{
	...
	nvt_sdp_drv_remove(&pdrv_info->module_info);
...
static int nvt_sdp_suspend( struct platform_device * pdev, pm_message_t state)
{
	...
	pdrv_info = platform_get_drvdata(pdev);
	nvt_sdp_drv_suspend(&pdrv_info->module_info);
...
static int nvt_sdp_resume( struct platform_device * pdev)
{
	...
	pdrv_info = platform_get_drvdata(pdev);
	nvt_sdp_drv_resume(&pdrv_info->module_info);

    nvt_dbg(INFO, "finished\n");
	return 0;
}

static struct platform_driver nvt_sdp_driver = {
    .driver = {
		        .name   = "nvt_sdp",
				.owner = THIS_MODULE,
				.of_match_table = sdp_match_table,
		      },
    .probe      = nvt_sdp_probe,
    .remove     = nvt_sdp_remove,
	.suspend = nvt_sdp_suspend,
	.resume = nvt_sdp_resume
};
...
int __init nvt_sdp_init(void)
{
    ...
    nvt_dbg(WRN, "\n");
	ret = platform_driver_register(&nvt_sdp_driver);
...
{
    nvt_dbg(WRN, "\n");
	platform_driver_unregister(&nvt_sdp_driver);
}

module_init(nvt_sdp_init);
module_exit(nvt_sdp_exit);

MODULE_AUTHOR("Novatek Corp.");
MODULE_DESCRIPTION("sdp driver");
MODULE_LICENSE("GPL");
