...
*/
	...
	case VENDOR_AIS_FLOW_IOC_IS_PRELOAD_MODEL:
		{
			INT is_preload_model = 0;
			is_preload_model = kdrv_ai_is_preload_model();
			if (unlikely(copy_to_user(argp, &is_preload_model, sizeof(INT)))) {
				return -EFAULT;
			}
			break;
		}
